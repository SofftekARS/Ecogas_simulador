...
@section('header')
    <div class="page-header">
        <h1><i class="glyphicon glyphicon-edit"></i> FacturaMinimas / Edit #{{$factura_minima->id}}</h1>
    </div>
@endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <form action="{{ route('factura_minimas.update', $factura_minima->id) }}" method="POST">
                <input type="hidden" name="_method" value="PUT">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">

                <div class="form-group @if($errors->has('categoria')) has-error @endif">
                       <label for="categoria-field">Categoria</label>
                    <input type="text" id="categoria-field" name="categoria" class="form-control" value="{{ is_null(old("categoria")) ? $factura_minima->categoria : old("categoria") }}"/>
                    ...
                    <input type="text" id="porcentaje-field" name="porcentaje" class="form-control" value="{{ is_null(old("porcentaje")) ? $factura_minima->porcentaje : old("porcentaje") }}"/>
